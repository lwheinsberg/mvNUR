...
Most of our data (created in the [data preparation](./prep.qmd) section) are directly analyzed by the mvBIMBAM command line program (see below) and not formerly loaded into R. Here though, we will read in our stored lists of phenotypes/variants of interest. See the [introduction section](./intro.qmd) for information about phenotypes and SNPs of interest.
...
-f: number of phenotypes to analyze (length(traits), for our example this number is 6) -A: indicate multiple values for sigma (see mvBIMBAM manual for more details; for our example we will use a range of values from 0.05 to 0.4)\
...
We now use the 'system' command to run the mvBIMBAM program on our data set, which outputs the following messages:
